...
# append file_name to each row (will be used for date)  - the play.log.fn file is with one more column:file name
cd ../data/play/
for f in *.log
do
 echo "Processing $f"
...
# cat all log with filename to one file (combine all the play.log.fn to one file)
